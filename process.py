...
def process_video(url, start_time=None, duration=None, last_seconds=None, output_name="clip_result.mp4", project_type="public"):
    ...
    master_dir = os.path.join(root_dir, "youtube", "masters") # Centralized masters to save space
    ...
    print(f"--- Project: [{project_type.upper()}] | Video: {url} ---")
    ...
    if is_long_video:
        print(f">>> Strategy: Master + Clip")
        master_file = os.path.join(master_dir, f"master_{vid_id}.mp4")
        if not os.path.exists(master_file):
            print(f"Downloading Master to Central Storage...")
        ...
        print(f">>> Strategy: Direct Streaming")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project", choices=["public", "private"], default="public", help="Project type (public or private)")
